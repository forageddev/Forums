...
object MeetupKillsLeaderboard : Leaderboard<Int>("Kills")
{
    override fun updateAndRefreshCache() {
        ...
        CgsPlayerHandler.handle.loadAll(DataStoreStorageType.MONGO).thenAccept {
            it.values.filterNot { (it.gameSpecificStatistics["UhcMeetupCgsStatistics"]?.kills?.value ?: -1) == -1 }.sortedByDescending {
                it.gameSpecificStatistics["UhcMeetupCgsStatistics"]?.kills?.value ?: -1
            }.stream().limit(10).collect(Collectors.toList()).forEachIndexed { index, player ->
                sortedMap[ProfileService.fetchProfile(player.identifier)] = index to (player.gameSpecificStatistics["UhcMeetupCgsStatistics"]?.kills?.value ?: -1)
            }
        }
    }
}