...
object MeetupWinsLeaderboard : Leaderboard<Int>("Wins")
{
    override fun updateAndRefreshCache() {
        ...
        CgsPlayerHandler.handle.loadAll(DataStoreStorageType.MONGO).thenAccept {
            it.values.filterNot { (it.gameSpecificStatistics["UhcMeetupCgsStatistics"]?.wins?.value ?: -1) == -1 }.sortedByDescending {
                it.gameSpecificStatistics["UhcMeetupCgsStatistics"]?.wins?.value ?: -1
            }.stream().limit(10).collect(Collectors.toList()).forEachIndexed { index, player ->
                sortedMap[ProfileService.fetchProfile(player.identifier)] = index to (player.gameSpecificStatistics["UhcMeetupCgsStatistics"]?.wins?.value ?: -1)
            }
        }
    }
}